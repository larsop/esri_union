-- TODO call single union tread with parameter to remove duplicated code
-- TODO fix handling of removed grid cell
DROP FUNCTION IF EXISTS get_esri_union_muti_thread (input_table_one text, input_table_two text,result_table_name text,max_rows_pr_cell int,tmp_grid_table_name text,boolean); 

CREATE OR REPLACE FUNCTION get_esri_union_muti_thread (
...
run_sql_created boolean DEFAULT true,
remove_grid_lines boolean DEFAULT false) RETURNS SETOF text AS
$body$
DECLARE
...
BEGIN
	...
	FOR cell_id IN 1..max_cell_id LOOP
		...
		ELSE
			func_call := 'SELECT get_esri_union_cell(' || quote_literal(singel_paramter) || ')';
	...
	IF run_sql_created = true AND remove_grid_lines = true THEN		
...
GRANT EXECUTE ON FUNCTION get_esri_union_muti_thread (
input_table_one text, 
input_table_two text,
